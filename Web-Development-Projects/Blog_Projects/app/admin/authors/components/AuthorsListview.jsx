...
import { useAuthors } from "@/lib/firebase/authors/read";
import { deleteAuthors } from "@/lib/firebase/authors/write";

// ================= TABLE SKELETON =================
function AuthorsTableSkeleton({ rows = 5 }) {
  return (
    <>
...
export default function AuthorsListview() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteAuthors(selectedCat);
      toast.success("Authors deleted successfully");
      setOpenDialog(false);
      setSelectedCat(null);
    } catch (err) {
      toast.error(err.message || "Failed to delete authors");
    }
  };
  ...
  return (
    <Card className="w-full  mx-auto overflow-hidden h-fit">
      ...
      <CardContent>
        {isLoading ? (
          <AuthorsTableSkeleton rows={pageSize} />
        ) : error ? (
                  ...
                  <TableRow key={cat.id}>
                    ...
                    <TableCell className="p-2 flex justify-center items-center">
                      ...
                      <div className="hidden lg:flex gap-2">
                        <Link href={`/admin/authors/form?id=${cat.id}`}>
